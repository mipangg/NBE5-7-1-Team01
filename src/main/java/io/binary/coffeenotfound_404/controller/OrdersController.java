...
public class OrdersController {
    ...
    public ResponseEntity<Orders> update(@PathVariable String email, @PathVariable Long ordersId
        ...
        return ResponseEntity.status(HttpStatus.OK)
                .body(ordersService.updateByEmailAndId(email, ordersId, request));
    }

    @DeleteMapping("/{email}/{ordersId}")
    public ResponseEntity<Void> delete(@PathVariable String email, @PathVariable Long ordersId) {
        ordersService.deleteByEmailAndId(email, ordersId);
